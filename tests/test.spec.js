...
module.exports = {
  'Should have ionic header bar': function (browser) {
    browser
      .url(browser.launch_url)
      .verify.elementPresent('ion-header-bar')
      .end()
  },
  'Should Display header bar title as News': function (browser) {
    browser
      .url(browser.launch_url)
      .verify.containsText('ion-header-bar','News')
      .end()
  },

  'Should display news on load sorted by published time(new to old)': function (browser) {
    browser
      .url(browser.launch_url)
      .verify.elementPresent('.article-image')
      .verify.elementPresent('.article-title')
      .verify.elementPresent('.article-author')
      .verify.elementPresent('.article-time')
      .elements('css selector', '.article-image', function (result) {
        browser.verify.equal(result.value.length, articles.length)
        for (let i = 0; i < result.value.length; i++) {
          browser.elementIdLocationInView(result.value[i].ELEMENT, (res) => {
            browser.elementIdAttribute(result.value[i].ELEMENT, "src", function (ele) {
              if (ele.value.trim().includes(articles[i].urlToImage.trim())) {
      ...
      .elements('css selector', '.article-title', function (result) {
        browser.verify.equal(result.value.length, articles.length)
        for (let i = 0; i < result.value.length; i++) {
          browser.elementIdLocationInView(result.value[i].ELEMENT, (res) => {
            browser.elementIdText(result.value[i].ELEMENT, (ele) => {
              if (ele.value.trim().includes(articles[i].title.trim())) {
                browser.verify.equal(true, true)
              } else {
                browser.verify.equal(true, false)
              }
      ...
      .elements('css selector', '.article-author', function (result) {
        browser.verify.equal(result.value.length, articles.length)
        for (let i = 0; i < result.value.length; i++) {
          browser.elementIdLocationInView(result.value[i].ELEMENT, (res) => {
            browser.elementIdText(result.value[i].ELEMENT, (ele) => {
              if (ele.value.trim().includes(articles[i].author.trim())) {
                browser.verify.equal(true, true)
              } else {
                browser.verify.equal(true, false)
              }
      ...
      .elements('css selector', '.article-time', function (result) {
        browser.verify.equal(result.value.length, articles.length)
        for (let i = 0; i < result.value.length; i++) {
          browser.elementIdLocationInView(result.value[i].ELEMENT, (res) => {
            browser.elementIdText(result.value[i].ELEMENT, (ele) => {
              if (ele.value.trim().includes(articles[i].publishedAt.trim())) {
                browser.verify.equal(true, true)
              } else {
                browser.verify.equal(true, false)
              }
